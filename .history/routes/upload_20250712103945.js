...
// Upload boarding pass image (guest-friendly)
router.post('/boarding-pass-guest', upload.single('boardingPass'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ error: 'No file uploaded' });
        }
        ...
        // Store file metadata in database with guest flag
        const fileData = {
            id: fileId,
            originalName: req.file.originalname,
            fileName: fileName,
